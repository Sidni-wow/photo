			...
			if(!empty($email) AND !empty($pass) AND !empty($name) AND !empty($alias)){
				if((strlen($email) <= 40) AND (strlen($pass) <= 40) AND (strlen($name) <= 40) AND (strlen($alias) <= 40)){
					...
					if($verifyEmail == 1){
						...
						$stmt = $this->pdo->prepare("INSERT INTO `users` (`id`, `email`, `pass`, `xp`, `my_like`, `lvl`, `name`, `alias`, `my_subscribers`, `i_subscriber`, `web_site`, `count_photo`, `status`, `minAva`)
													VALUES (NULL, :email, :pass, '0', '0', '0', :name, :alias, :defoult, :defoult, '', '0', '', :defaultAva)");
						$stmt->execute(array(
							':email' => $email,
							':pass' => $pasHash,
							':name' => $name,
				...
				if((strlen($email) <= 40) AND (strlen($pass) <= 40)){
						...
						while($row = $stmt->fetch(PDO::FETCH_LAZY)){
							...
							if(password_verify($pass,$row['pass'])){
								...
								//$text = "Неправельные данные1";
							}
						}
						//$text = "Неправельные данные2";
				}else{
					//$text = "Неправильная длинна3";
				}
			}else{
				//$text = "Заполните все поля4";
			}
			//echo '<br />'.$text;
			header("Location:?option=login");
			exit;
